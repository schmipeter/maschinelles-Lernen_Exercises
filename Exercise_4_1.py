...
cats_w = scipy.io.loadmat(io.BytesIO(requests.get(f"{base}/catData_w.mat").content))[
    "cat_wave"
]
dogs_w = scipy.io.loadmat(io.BytesIO(requests.get(f"{base}/dogData_w.mat").content))[
    "dog_wave"
]
...
def roundtrip(clf, name):
    ...
    onx_train = accuracy_score(
        y_train, sess.run(None, {xkey: X_train.astype(np.float32)})[0]
    )
    onx_test = accuracy_score(
        y_test, sess.run(None, {xkey: X_test.astype(np.float32)})[0]
    )
...
def voting(est, vote, **kw):
    return make_pipeline(
        PCA(n_components=41),
        VotingClassifier(estimators=est, voting=vote, flatten_transform=False, **kw),
    )


models = {
    "orig_soft_SVCprob": voting(
        [
            ("lda", LDA()),
            (
                "rf",
                RandomForestClassifier(
                    n_estimators=500, max_leaf_nodes=2, random_state=6020
                ),
            ),
            ("svc", SVC(kernel="linear", probability=True, random_state=6020)),
        ],
        vote="soft",
    ),
    "hard_SVC": voting(
        [
            ...
            ("svc", SVC(kernel="linear", random_state=6020)),
        ],
        vote="hard",
    ),
    "hard_LinearSVC": voting(
        [
            ("lda", LDA()),
            (
                "rf",
    ...
    "soft_RF+LR": voting(
        [
            ...
            ("lr", LogisticRegression(max_iter=1000, random_state=6020)),
        ],
        vote="soft",
    ),
}
