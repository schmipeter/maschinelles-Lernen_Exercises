...
for i, metric in enumerate(metrics):
    for j, eps in enumerate(eps_values):
        ...
        ax = axes[i, j]
        sc = ax.scatter(
            X_moons[:, 0], X_moons[:, 1], c=labels, cmap="viridis", edgecolor="k", s=30
        )
...
print(
    f"DBSCAN on Iris dataset: Found {n_clusters} clusters with {n_noise} noise points."
)
...
# DBSCAN
#   Two-Moons:
#     – eps=0.3 trennt Halbmonde perfekt (egal ob euklidisch oder Manhattan).
#     – Zu kleines eps (0.1) zerfasert in viele Mini-Cluster.
#     – Zu großes eps (0.5) vereint alles zu einem Cluster.
#
#   Iris:
#     – Mit eps=0.8 und min_samples=5 entstehen 2 Haupt-Cluster + wenige Ausreißer.
#     – Ergebnis ähnelt der natürlichen Trennung Setosa vs. Versicolor/Virginica.
#
#   DBSCAN findet nicht-konvexe Cluster ohne k-Vorgabe, erfordert aber
#   sorgfältiges Tuning von eps; zu klein → Over-segmentation, zu groß → Alles eins.
