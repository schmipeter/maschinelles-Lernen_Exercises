...
X_test = np.r_[
    cats[TRAIN_SIZE : TRAIN_SIZE + TEST_SIZE // 2],
...
def make_model(svc_kernel="linear", svc_prob=True):
    ...
    rf = RandomForestClassifier(
        n_estimators=RF_TREES, max_leaf_nodes=2, random_state=42
    )
    est = [("lda", LDA()), ("rf", rf), ("svc", svc)]
    return make_pipeline(
        PCA(n_components=PCA_NCOM),
        VotingClassifier(estimators=est, voting="hard", flatten_transform=False),
    )
...
def kb(p):
    return f"{Path(p).stat().st_size//1024} KB"
